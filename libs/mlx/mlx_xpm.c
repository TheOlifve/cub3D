...
void	*mlx_int_gamee_xpm(mlx_ptr_t *xvar,void *info,int info_size,char *(*f)())
{
  int	pos;
  char	*line;
  char	**tab;
...
void	*mlx_xpm_file_to_image(mlx_ptr_t *xvar,char *file,int *width,int *height)
{
  ...
  if ((img = mlx_int_gamee_xpm(xvar,ptr,size,mlx_int_get_line)))
    {
      *width = img->width;
      *height = img->height;
    }
...
void	*mlx_xpm_to_image(mlx_ptr_t *xvar,char **xpm_data,int *width,int *height)
{
  ...
  if ((img = mlx_int_gamee_xpm(xvar,xpm_data,0,mlx_int_static_line)))
    {
      *width = img->width;
      *height = img->height;
    }
